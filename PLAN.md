...
>
> **Compose-vs-raw speed mystery — RESOLVED 2026-08-04: it's GPU thermal throttling, not deployment config.** Rerunning the identical image/command that once measured 14–20s now takes 90–120s; every configuration (compose, raw, old image, new image, cold start) measures slow on a heat-affected card. Telemetry during runs: `SW Thermal Slowdown: Active`, SM clocks collapsing 1600→450–1200 MHz within ~10s of load, while core temp is only 75–79°C (below the 83°C target), power ~205–260W (below the 300W cap), and the fan already at 100%. Ruled out: PCIe degradation (Gen4 ×16 confirmed under load), locked/application clocks, the watchdog, power-cap throttling, software stack. The signature — driver throttling with an innocent-looking core sensor and maxed fan — is the classic 3090 **GDDR6X memory-junction** pattern (sensor not exposed on GeForce, runs 20–30°C above core). 77°C core at 240W on a 100% fan also indicates poor heat transfer (aged pads/paste or case airflow). **The card has ~15s/16-min diarization capability when thermally healthy; sustained-load throttling taxes it ~5×. The fix is physical (repad/repaste/airflow), not software.** A 25-min deep-cooldown A/B (to confirm memory-heat hysteresis explains the one fast window) was designed but not run — worth doing before any hardware work.
...
>
> **Parakeet residency solved (2026-08-04, commit 8218726):** the parakeet compose service (3.3 GB resident) was replaced by a llama-swap entry (`parakeet-tdt-0.6b-v3` in `~/sandbox/llama-swap/config.yaml`) that runs the same container on demand with a 300s TTL. Cold start + 16-min transcription: 8s. Idle GPU footprint of the whole service is now ~0.6 GB. Note: parakeet.cpp's health endpoint is `/health` (`/v1/models` 404s) — llama-swap's `checkEndpoint` must match.
>
